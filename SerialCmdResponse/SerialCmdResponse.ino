...
int Trim(char* ch, const int CharCnt, char toRemove);
void ToUpper(char* ch, const int CharCnt);
int Convert_ToStringArray(const char* strInbuf, int StrLen, char SplitChar, const int MaxElement, String* OutStringArray);
bool Contains(String MainStr, String SeekStr, uint8_t CharStartIndex = 0);
...
void setup() {
  ...
  Serial.println("Enter String Command");
}
...
void loop() 
{
 char charCnt = 0; 
  ...
  charCnt = Serial.available();
  
  if (charCnt > 0) 
  {
    
    charCnt = Serial.readBytesUntil('>', cbuf, charCnt);  
    ...
    Trim(cbuf, charCnt, '<');
    Trim(cbuf, charCnt, '>');
    ToUpper(cbuf, charCnt);
    String strOutBuf[MAX_STRING_COUNT];
    
    if (Contains(cbuf, ","))
    {
      strListCnt = Convert_ToStringArray(cbuf, charCnt, ',', MAX_STRING_COUNT, strOutBuf);
    }
    else
    {
      String stmp(cbuf);
      //Serial.print("Rx:"); Serial.println(cbuf); //for debug
      strOutBuf[0] = stmp;
    }


   //-------------------------------
   if (Contains(strOutBuf[0],"APPL"))
  {
    Serial.println("APPL command Received!");
  } 
  //---------------------------------------------------
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "ACWL"))
  {
    Serial.println("ACWL command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "ACLB"))
  {
    Serial.println("ACLB command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "ASTW"))
  {
    Serial.println("ASTW command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "BTSW"))
  {
    Serial.println("BTSW command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA01"))
  {
    Serial.println("CA01 command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA02"))
  {
    Serial.println("CA02 command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA03"))
  {
    Serial.println("CA03 command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA04"))
  {
    Serial.println("CA04 command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA05"))
  {
    Serial.println("CA05 command Received!");
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA06"))
  {
    Serial.println("CA06 command Received!");
    
  }
  //---------------------------------------------------
  else if (Contains(strOutBuf[0], "CA07"))
  {
    Serial.println("CA07 command Received!");
  }
...
int Convert_ToStringArray(const char* strInbuf, int StrLen, char SplitChar, const int MaxElement, String* OutStringArray)
  {
    ...
    //OutStringArray = new cString[MaxElement];

    while (ix < StrLen)
    {
      String s = "";

      for (int i = ix; i < StrLen; i++)
      {

        if (strInbuf[i] == SplitChar || strInbuf[i] == '\n' || strInbuf[i] == '\r')
        {
          ix++;
          break;
        }
      ...
      if (s != "" && StrArrayCount < MaxElement)
      {
        OutStringArray[StrArrayCount] = s;
        StrArrayCount++;
      }
 ...
 bool Contains(String MainStr, String SeekStr, uint8_t CharStartIndex)
 {
    int scrlen = MainStr.length();
    int seeklen = SeekStr.length();
    int checklen = 0;
    
    for (int i = CharStartIndex; i < scrlen; i++)
    {
      if (MainStr[i] == SeekStr.charAt(checklen))
      {
        checklen++;
        if (checklen == seeklen)
        {
